...
import {
  generateMemoryDraft, getMemoryVersion, latestMemory, listMemoryVersions, saveMemoryDraft
...
export function useMemoryVersions(projectId: string | undefined) {
  return useQuery({
    ...
    queryFn: () => (projectId ? listMemoryVersions(projectId) : Promise.resolve<Memory[]>([])),
    enabled: !!projectId
  });
}
...
export function useLatestMemory(projectId: string | undefined) {
  return useQuery({
    ...
    queryFn: () => (projectId ? latestMemory(projectId) : Promise.resolve<Memory | null>(null)),
    enabled: !!projectId
  });
}

export function useMemoryVersion(memoryId: string | undefined) {
  return useQuery({
    queryKey: memoryId ? ['memory', 'version', memoryId] : ['memory', 'version', '__none__'],
    queryFn: () => (memoryId ? getMemoryVersion(memoryId) : Promise.resolve<Memory | null>(null)),
    enabled: !!memoryId
  });
}
...
export function useGenerateDraft() {
  return useMutation({
    mutationFn: ({ projectId, mode, version, localPath, name }: {
      ...
      generateMemoryDraft(projectId, mode ?? 'fresh', version,
...
export function useSaveDraft() {
  ...
  return useMutation({
    mutationFn: ({ projectId, content, source }: { projectId: string; content: string; source?: MemorySource }) =>
      saveMemoryDraft(projectId, content, source ?? 'user-edited'),
    onSuccess: (_m, vars) => {
